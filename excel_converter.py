...
def flatten_job_data(job):
    ...
    """
    # job_scraper.py now writes flat rows; pass those straight through, minus
    # the optional raw payload. Older raw-shaped exports fall through below.
    if 'job_information' not in job:
        flat = {k: v for k, v in job.items() if k != 'raw'}
        flat['title'] = clean_html_text(str(flat.get('title', '')))
        return flat
    ...
    try:
        ...
        for i in range(0, len(df), chunk_size):
            ...
            with pd.ExcelWriter(filename, engine='openpyxl') as writer:
                ...
                chunk_metrics = [(f'Jobs in Part {part_number}', len(chunk))]
                for column, label in [
                    ('source', 'Unique Sources'),
                    ('company', 'Unique Companies'),
                ]:
                    if column in chunk.columns:
                        chunk_metrics.append((label, chunk[column].nunique()))
                for column, label in [
                    ('viewed_count', 'Total Viewed'),
                    ('applied_count', 'Total Applied'),
                    ('saved_count', 'Total Saved'),
                    ('hidden_count', 'Total Hidden'),
                ]:
                    if column in chunk.columns:
                        chunk_metrics.append((label, chunk[column].sum()))
                chunk_summary_df = pd.DataFrame(chunk_metrics, columns=['Metric', 'Value'])
        ...
        with pd.ExcelWriter(summary_filename, engine='openpyxl') as writer:
            ...
            metrics = [
                ('Total Jobs', len(df)),
                ('Total Excel Files', total_chunks),
                ('Jobs per File (Max)', chunk_size),
            ]
            # Only report on columns this export actually has.
            for column, label in [
                ('source', 'Unique Sources'),
                ('company', 'Unique Companies'),
                ('location', 'Unique Locations'),
                ('workplace_type', 'Unique Workplace Types'),
            ]:
                if column in df.columns:
                    metrics.append((label, df[column].nunique()))
            for column, label in [
                ('viewed_count', 'Total Viewed'),
                ('applied_count', 'Total Applied'),
                ('saved_count', 'Total Saved'),
                ...
                if column in df.columns:
                    metrics.append((label, df[column].sum()))
            if 'apply_url' in df.columns:
                metrics.append(('Jobs With Apply URL', int(df['apply_url'].astype(bool).sum())))

            summary_df = pd.DataFrame(metrics, columns=['Metric', 'Value'])
            summary_df.to_excel(writer, sheet_name='Overall Summary', index=False)

            # Top companies (top 50)
            company_column = 'company' if 'company' in df.columns else 'source_and_board_token'
            top_companies = df[company_column].value_counts().head(50)
